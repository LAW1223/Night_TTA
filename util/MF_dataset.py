...
class MF_dataset_Triple(Dataset):

    def __init__(self, data_dir, split, input_h=480, input_w=640, transform=[]):
        super(MF_dataset_Triple, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        label = np.asarray(PIL.Image.fromarray(label).resize((self.input_w, self.input_h), resample=PIL.Image.NEAREST))
        label = label.astype('int64')
        label_binary=  np.where(label>0,1,0)

        return torch.tensor(image), torch.tensor(label), torch.tensor(label_binary),name
