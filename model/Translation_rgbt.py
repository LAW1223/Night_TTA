...
class OURNet(nn.Module):
    ...
    def forward(self, input):
        ...
        ######################################################################
        # decoder rgb
        rgb_result = self.deconv1(rgb)
        rgb_result = self.deconv2( rgb_result)
        rgb_result = self.deconv3( rgb_result)
        rgb_result = self.deconv4( rgb_result)
        rgb_result = self.deconv5( rgb_result)

        # decoder thermal
        thermal_result = self.deconv1(thermal)
        thermal_result = self.deconv2(thermal_result)
        thermal_result = self.deconv3(thermal_result)
        thermal_result = self.deconv4(thermal_result)
        thermal_result = self.deconv5(thermal_result)
...
def unit_test():
    num_minibatch = 2
    input = torch.randn(num_minibatch, 4, 240, 320).cuda(0)
    rtf_net = OURNet(9).cuda(0)
    y1,y2,y3=rtf_net(input)
    print(y1.shape)
    print(y2.shape)
    print(y3.shape)
